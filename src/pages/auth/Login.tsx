...
// UserRole từ BE: Guest=0, Member=1, Staff=2, Admin=3
const UserRole = { Guest: 0, Member: 1, Staff: 2, Admin: 3 } as const;
...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      login(res.result.access_token, res.result.refresh_token);

      // Redirect theo role (Member -> trang chủ; Staff/Admin -> trang chủ, đồng đội sẽ thêm route sau)
      const redirectTo = res.redirectTo || '/';
      if (res.result.role === UserRole.Member || res.result.role === UserRole.Staff || res.result.role === UserRole.Admin) {
        navigate('/');
      } else {
        navigate(redirectTo.startsWith('/') ? redirectTo : '/');
      }
    } catch (err: unknown) {
      const msg = (err as { message?: string })?.message || 'Tài khoản hoặc mật khẩu không đúng.';
      setError(msg);
    } finally {
      setLoading(false);
    }
